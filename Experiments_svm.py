...
def run_algorithm(data_points, w):
    # fit the One-Class SVM model
    # train-test split
    #X_train, X_test = train_test_split(data_points, test_size=0.2, random_state=42)
    ...
    good_data_points = data_points[y_pred == 1].tolist()
    outlier_buffer = OutlierBuffer(w)
    outliers_detected = data_points[y_pred == -1]
    for outlier in outliers_detected:
    ...
    sorted_outliers_svm_list = sorted_outliers.tolist()
    sorted_outliers_svm_list = [item for item in sorted_outliers_svm_list if item not in filtered_outliers.tolist()]
    return xmin_outlier, xmax_outlier, ymin_outlier, ymax_outlier, len(good_data_points), good_data_points, sorted_outliers_svm_list,accuracy
...
for size in sizes:
    ...
    for p in range(num_loops):
        ...
        #Algorithm
        xmin_outlier, xmax_outlier, ymin_outlier, ymax_outlier, num_matching_points, good_data_points, sorted_outliers,accuracy= run_algorithm(data_points, w)
